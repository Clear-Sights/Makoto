"""pattern 1.22 — fabricated commit SHA/tag presented as proof of a commit.
...
produced it. The SHA is hallucinated, the same way pattern 1.9 catches a
...
Detection (FP-SAFE BY DESIGN):
  1. StopCheck to a real end-of-turn Stop claim. We read the AI's closing text from
     ...
     see lib.claims.claims_done and docs/ADVERSARY-FINDINGS.md). This is
  ...
  4. If the claimed SHA literally appears in any prior tool_result payload
     ...
     -> NEVER fire. The SHA is grounded in real tool output (mirrors 1.9's
...
# ---------------------------------------------------------------------------
# REVISION (1.22, verity-1.22-revise): require a POSITIVE COMMIT-CLAIM, exclude
...
_NEG_REF_RX = re.compile(
    ...
    # commit evidence — 1.22's cheat class is the AI's own claimed work.
...
# invocation form. WIDENED (1.22 revision, 2026-05-29) to close the documented
...
def _stop_text(current_event: dict) -> str:
    ...
    1759 captured Stop events; see lib.claims.claims_done +
    docs/ADVERSARY-FINDINGS.md). We therefore:
