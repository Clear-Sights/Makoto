"""content.jwt_signature_disabled predicate — JWT signature verification DISABLED (active-code only).
...
classic auth bypass. This is the jwt case content.cert_verify_disabled deliberately punts (content.cert_verify_disabled's TLS callee gate
...
Active-code only (substrate.factories.parse_introduced): a comment / string / docstring that merely MENTIONS
...
# CALLEE GATE (shared via factories.jwt_decode_callee_chain, also used by content.jwt_none_alg -- extracted
# 2026-07-09 from two hand-duplicated copies): the bare keyword `verify=False` is not jwt-specific,
# so (mirroring content.cert_verify_disabled's TLS gate) it only fires when the call's CALLEE chain names a JWT
...
CHECK = _Check(id='content.jwt_signature_disabled', applies_at="Pre", posture="BLOCK", predicate_module=__name__, keywords=('verify=False', 'verify_signature'), retry_hint=RETRY_HINT, description=DESCRIPTION)
