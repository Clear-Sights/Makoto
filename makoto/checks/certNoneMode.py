"""content.cert_none_mode predicate — certificate verification disabled via verify_mode = CERT_NONE
(active-code only).
...
assignment content.cert_verify_disabled deliberately punts: a BARE ``ssl.CERT_NONE`` is also the right-hand side of a
legitimate COMPARISON (``if mode == ssl.CERT_NONE:``), so content.cert_verify_disabled does not match CERT_NONE at all. The
...
Active-code only (substrate.factories.parse_introduced): a comment / string / docstring mentioning the shape
