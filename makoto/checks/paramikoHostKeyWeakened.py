"""content.paramiko_host_key_weakened predicate — paramiko SSH host-key verification weakened to auto-add / warn-only.
...
NOT match. Active-code only (substrate.factories.parse_introduced): a comment / docstring mention never fires; a
...
CHECK = _Check(id='content.paramiko_host_key_weakened', applies_at="Pre", posture="BLOCK", predicate_module=__name__, keywords=('set_missing_host_key_policy', 'AutoAddPolicy', 'WarningPolicy', 'paramiko'), retry_hint=RETRY_HINT, description=DESCRIPTION)
