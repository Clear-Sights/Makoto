...
The following entries exist so that internal Makoto documentation (specs, design docs, ADRs) can illustrate the citation-capture flow with real-world references without tripping `content.phantom_citation` on the doc files themselves.
...
- This configured file is the sole population source. On the next mtime-gated refresh, Makoto rebuilds `canonical_citations` from its contents.
