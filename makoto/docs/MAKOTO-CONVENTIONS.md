...
- `gate.contract_order` — a result-producing tool call issued while a declared Plan's dependency for that step is still undischarged (SPEC-5, ported by shape from Assay's `ContractOrder`); its Stop-time sibling gate of the same id guards the remainder at turn end. The evidence is the plan store's own recorded order, not file content. A Plan is declared by writing JSONL nodes (`{"what","passthrough","where"[,"id"]}` per line) to `.claude/makoto-plan.jsonl` — either staged on disk before the session starts, or via a live `Write`/`Edit`/`MultiEdit`/`NotebookEdit` to that same path mid-session (latest-wins: a later write replaces the whole Plan). A node advances to DONE automatically the moment a `Write`/`Edit`/`MultiEdit`/`NotebookEdit` targets that node's own `where`; Stop only blocks while open nodes remain, so an undischarged Plan is never a dead end — finish the node's `where`, or redeclare the Plan without it.
