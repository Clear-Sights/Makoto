...
# plugin's own package the first candidate.  A missing plugin root makes the event unevaluable;
# refuse it loudly rather than claiming a successful hook evaluation.
...
if [ -z "${CLAUDE_PLUGIN_ROOT:-}" ] || ! cd "$CLAUDE_PLUGIN_ROOT" 2>/dev/null; then
  echo "makoto _dispatch_shim: CLAUDE_PLUGIN_ROOT unset or not a directory -- NOT_EVALUABLE" >&2
  exit 2
fi
