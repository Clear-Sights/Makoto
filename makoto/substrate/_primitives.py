...
Relocated here (SPEC-5 Task 2, 2026-07-06) from the former top-level `makoto/checks.py`
module: the name `makoto.checks` is now the flat detector-catalog PACKAGE (`_loader.py`'s
`load_checks()`), so this module moved intra-package as `_primitives.py` (underscore-prefixed,
like `_loader.py`, so the loader's directory scan never mistakes it for a detector module).
`makoto/checks/__init__.py` re-exports every name below verbatim, so every pre-existing
`from makoto.checks import normalize_path`-shaped call site is unaffected — logic here is
byte-for-byte unchanged from the old `checks.py`.
"""
import os
import re
