...
Every check module landed in this package adds ONE entry here, alongside dropping in
its own `.py` file -- the two are meant to move together; `checks.catalogCompleteness` audits
...
It contains exactly today's live catalog so the completeness check reports zero drift at rest.
"""
DECLARED_IDS: dict = {
    "gate.catalog_completeness": "catalogCompleteness",
