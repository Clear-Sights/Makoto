...
def test_io_exports_renamed_symbols():
    from makoto.substrate import io as mio
...
def test_io_is_L1_imports_only_L0():
    src = Path(__file__).resolve().parents[2] / "makoto" / "substrate" / "io.py"
    tree = ast.parse(src.read_text())
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom) and node.module and node.module.startswith("makoto"):
            assert node.module in {"makoto.core.lexicons", "makoto.core._shell"}, (
                f"L1 io may import only L0 helpers: {node.module}")
