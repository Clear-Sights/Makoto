...
def test_claims_behaviour_preserved():
    from makoto.lib.claims import claims_done, claims_success
...
def test_claims_is_L1_imports_only_L0():
    src = Path(__file__).resolve().parents[2] / "lib" / "claims.py"
    tree = ast.parse(src.read_text())
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom) and node.module and node.module.startswith("makoto"):
            assert node.module == "makoto.lexicons", f"L1 claims may import only L0 lexicons: {node.module}"
...
def test_claims_done_positive():
    ...
    from makoto.lib.claims import claims_done
