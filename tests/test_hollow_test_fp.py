...
#   makoto: 0 fires. The public sync deliberately ships only tests/test_smoke.py (the full
#     falsifiability suite, including the former test_install.py true-positive, remains dev-only).
#   assay: 0 fires (all 6 pre-fix fires were the `_clean`/`_reason` shared-assert-helper FP, now fixed).
#   ventura's former tests/test_agent_dispatch.py true-positive disappeared with the legacy
#     AgentToolDispatcher surface; dispatch now lives under anthropic_dispatch/dispatcher_protocols.
...
def _annotated_hollow_test_functions(files: list[str]) -> set[str]:
    """The corpus-local, on-the-record positives for the raw hollow-test analyzer.

    `makoto-allow` is deliberately placed on a hollow test's ``def`` line: it is both the
    adapter's exemption anchor and a discoverable declaration that this otherwise-hollow test
    is intentional.  Derive the expected raw findings from those declarations, rather than
    pinning a dev-only function name into every corpus that imports this test module.
    """
    expected = set()
    for filename in files:
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)) \
                    ...
                    and "makoto-allow" in lines[node.lineno - 1].lower():
...
def test_corpus_fp_makoto_own_tests():
    ...
    # the live adapter applies the exemption.  Everything else remains a measured zero-FP set.
    expected = _annotated_hollow_test_functions(files)
    assert fires_by_func == expected, (
