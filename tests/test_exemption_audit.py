...
def test_exempted_real_match_is_recorded(tmp_path):
    ...
    out = _run("1.26", _evt("client.py", content), conn)
...
def test_pure_unit_call_without_conn_writes_nothing(tmp_path, monkeypatch):
    ...
    out = _run("1.26", _evt("client.py", content), None)
    assert out is None
...
def test_regex_factory_path_also_records(tmp_path):
    ...
    out = _run("1.4", _evt("config.toml", content), conn)
    assert out is None
    rows = list(audit.read_exemptions(st))
    assert len(rows) == 1 and rows[0]["pattern_id"] == "1.4"
...
def test_append_exemption_round_trips_through_reader(tmp_path):
    ...
    st = _state(tmp_path)
    audit.append_exemption(st, pattern_id="1.30", kind="makoto-allow", file="h.py", line=4,
                           reason="constant-time compare not needed here", snippet="a == b")
    rows = list(audit.read_exemptions(st))
    assert len(rows) == 1 and rows[0]["pattern_id"] == "1.30" and rows[0]["line"] == 4
