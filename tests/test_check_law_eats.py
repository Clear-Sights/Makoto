...
def derived_reads() -> dict[tuple[str, str], frozenset[str]]:
    ...
    for key, row in rows().items():
        fields = PRE_CONTEXT_FIELDS if row.edge == "Pre" else STOP_CONTEXT_FIELDS
        context_names = None
        if row.edge != "Pre" and isinstance(row.root, (ast.Lambda, ast.FunctionDef, ast.AsyncFunctionDef)):
            args = [*row.root.args.posonlyargs, *row.root.args.args]
            context_names = {args[0].arg} if args else set()
        derived = _walk_reads(row.root, row.root_funcs, fields, set(), context_names)
        if (
            row.edge != "Pre"
            and _imports_discharge_kwargs(row.tree)
            and _calls_name(row.root, row.funcs, "_discharge_kwargs", set())
        ):
            derived |= DISCHARGE_EATS
        reads[key] = frozenset(derived)
    return reads
