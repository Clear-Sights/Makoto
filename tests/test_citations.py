...
def test_extract_citations_skips_iso_date():
    """a year directly followed by -DD is a DATE, not a citation (fixes the content.phantom_citation dated-heading FP)."""
    from makoto.state.citations import extract_citations
...
def test_extract_citations_filters_stopword_authors():
    ...
    Live audit log (May 2026) showed 40% FP rate on content.phantom_citation from this
