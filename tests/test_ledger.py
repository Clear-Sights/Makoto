"""Unit tests for makoto.state.ledger — update recording + read-by-key.
...
from makoto.state.ledger import record_update, read_key
...
def test_record_update_with_root_chain_appends_touched_row(tmp_path):
    """root=<explicit> (the real dispatch.py call site's shape) chain-appends a 'touched' row
...
def test_record_update_chain_fault_never_blocks_sqlite_write(tmp_path, monkeypatch):
    ...
    import makoto.state.ledger as _ledger_mod
    def _boom(*a, **k):
