...
# ---- BUGFIX (live-observed): a later, non-adjacent success for the SAME key must resolve it -----
# The original judged each closed run immediately and returned True on the first bad one,
# permanently, for the rest of the calls list -- so a retry loop that failed twice, had an
# unrelated call in between (e.g. checking something else while an API was flaky), and then
# retried the SAME call again and SUCCEEDED still read as stuck. Only the key's MOST RECENT run
# should decide its verdict.
def test_recur_silent_when_the_same_key_later_succeeds_after_an_unrelated_call_intervenes():
    ti = {"owner": "x", "repo": "y", "pullNumber": 8}
    calls = [
        _call(name="merge_pull_request", input=ti, result={"error": "503"}),
        _call(name="merge_pull_request", input=ti, result={"error": "503"}),   # bad run closes here
        _call(name="get_check_runs", input={"pullNumber": 8}, result={"stdout": "ok"}),  # different call
        _call(name="merge_pull_request", input=ti, result={"merged": True}),   # same key, now succeeds
    ]
    assert recur_stuck(calls) is False


def test_recur_still_fires_when_the_same_key_fails_again_after_an_unrelated_call_intervenes():
    ti = {"owner": "x", "repo": "y", "pullNumber": 8}
    calls = [
        _call(name="merge_pull_request", input=ti, result={"error": "503"}),
        _call(name="merge_pull_request", input=ti, result={"error": "503"}),
        _call(name="get_check_runs", input={"pullNumber": 8}, result={"stdout": "ok"}),
        _call(name="merge_pull_request", input=ti, result={"error": "503"}),
        _call(name="merge_pull_request", input=ti, result={"error": "503"}),   # still stuck
    ]
    assert recur_stuck(calls) is True
...
def test_recur_one_key_resolved_another_key_still_stuck():
    ...
    ti_b = {"command": "b"}
    calls = [
        _call(input=ti_a, result={"interrupted": True}),
        _call(input=ti_a, result={"interrupted": True}),
        _call(input=ti_a, result={"stdout": "ok"}),          # key a: resolved
        _call(input=ti_b, result={"interrupted": True}),
        _call(input=ti_b, result={"interrupted": True}),     # key b: still stuck
    ]
    assert recur_stuck(calls) is True
