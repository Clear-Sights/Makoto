...
# ---- declare_from_live_write (2026-07-23): the mid-session sibling of declare_from_session_artifact
# above -- same read/parse/falsifiability contract, no `source` gate (a live tool-call touch of the
# artifact IS the trigger, not a SessionStart sub-event).
def test_declare_from_live_write_absent_file_is_none(conn, tmp_path):
    assert plan_store.declare_from_live_write(str(tmp_path), "s1", conn) is None


def test_declare_from_live_write_admits_with_no_startup_gate(conn, tmp_path):
    (tmp_path / ".claude").mkdir()
    (tmp_path / ".claude" / "makoto-plan.jsonl").write_text(
        '{"what":"Write","passthrough":"a","where":"a.py"}\n'
    )
    got = plan_store.declare_from_live_write(str(tmp_path), "s1", conn)
    assert got is not None
    assert len(got.rows()) == 1
    assert plan_store.load_plan(conn, "s1") is not None


def test_declare_from_live_write_malformed_json_is_none(conn, tmp_path):
    (tmp_path / ".claude").mkdir()
    (tmp_path / ".claude" / "makoto-plan.jsonl").write_text("not json at all\n")
    assert plan_store.declare_from_live_write(str(tmp_path), "s1", conn) is None


def test_declare_from_live_write_non_falsifiable_node_rejects_whole(conn, tmp_path):
    (tmp_path / ".claude").mkdir()
    (tmp_path / ".claude" / "makoto-plan.jsonl").write_text(
        '{"what":"","passthrough":"a","where":"a.py"}\n'
    )
...
def test_declare_from_live_write_is_latest_wins_over_a_prior_declare(conn, tmp_path):
    """A second live write REPLACES the whole plan, matching declare_plan's own documented
    latest-wins semantics -- the SAME behavior a re-run SessionStart would never even attempt
    (declare_from_session_artifact only ever fires once, at a genuinely-new session)."""
    (tmp_path / ".claude").mkdir()
    artifact = tmp_path / ".claude" / "makoto-plan.jsonl"
    artifact.write_text('{"what":"Write","passthrough":"a","where":"a.py","id":"n1"}\n')
    plan_store.declare_from_live_write(str(tmp_path), "s1", conn)
    artifact.write_text('{"what":"Write","passthrough":"b","where":"b.py","id":"n2"}\n')
    got = plan_store.declare_from_live_write(str(tmp_path), "s1", conn)
    assert [n.id for n in got.nodes()] == ["n2"]
