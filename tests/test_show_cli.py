...
from makoto import ledger, __main__ as cli
...
def test_show_reads_ledger_row(tmp_path, monkeypatch, capsys):
    ...
    conn.close()
    monkeypatch.setattr("makoto.state._state_dir", lambda: state_dir)
...
def test_show_no_db_is_failsoft(tmp_path, monkeypatch, capsys):
    state_dir = tmp_path / "makoto_state"  # not created — no makoto.db
    monkeypatch.setattr("makoto.state._state_dir", lambda: state_dir)
