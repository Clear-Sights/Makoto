"""content.env_gated_audit sentinels — env-gated audit / verification code (active-code only).

content.env_gated_audit fires when a PreToolUse Write/Edit INTRODUCES, as REAL Python code (not a comment /
...
def test_neg_non_env_gate_with_audit_body():
    """An audit op gated on a NON-env condition (user.is_admin) -> silent. content.env_gated_audit requires an ENV read;
