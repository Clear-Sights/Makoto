"""content.self_mute_guard — makoto self-mute guard.
...
def test_fire_write_disable_gates_truthy():
    ...
    f = _run(p)
    assert f is not None and f.pattern_id == "content.self_mute_guard" and f.level == "error"


def test_fire_edit_disable_patterns_new_string():
    """MAKOTO_DISABLE_PATTERNS takes a LIST of check ids, not a boolean — ANY non-empty value
    is a selective self-mute and must fire. Regression: the truthy-only env regex used to pass
    this only by accident (the legacy numeric ids happened to start with '1'); a canonical-id
    list slipped through unseen until 2026-07-10."""
    p = _pre(_SETTINGS, new_string='"MAKOTO_DISABLE_PATTERNS": "content.verifier_predicate_weakened,content.self_mute_guard"', tool_name="Edit")
    assert _run(p) is not None


def test_fire_disable_patterns_shell_form():
    p = _pre(_SETTINGS, new_string="MAKOTO_DISABLE_PATTERNS=content.self_mute_guard", tool_name="Edit")
    assert _run(p) is not None


def test_silent_disable_patterns_emptied():
    """setting the var to '' is the RE-ENABLE move (mirrors the 0/false truthy-guard) — never fire."""
    p = _pre(_SETTINGS, content='{"env": {"MAKOTO_DISABLE_PATTERNS": ""}}')
...
def test_fire_global_disable_all_hooks_truthy():
    ...
    content.self_mute_guard's env/un-wire branches never inspect — a one-move total self-mute."""
...
def test_fire_guts_makoto_command_to_noop():
    ...
    p = _pre(_SETTINGS, old_string=old, new_string=new, tool_name="Edit")
    f = _run(p)
    assert f is not None and f.pattern_id == "content.self_mute_guard"
