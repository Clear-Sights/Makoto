"""content.verifier_body_hollowed sentinels — verifier HOLLOWED (body gutted to return-True / pass).

content.verifier_body_hollowed fires when a PreToolUse Write/Edit INTRODUCES, on the constitution integrity-check
surface (`constitution/integrity/checks/*.py` — the exact path content.verifier_predicate_weakened anchors on), a
...
checking. It is the deletion/short-circuit cousin of content.verifier_predicate_weakened (comparator-loosening) — the
refuter confirmed content.verifier_predicate_weakened does NOT cover it (content.verifier_predicate_weakened needs startswith/in[]/re.match, none present
in `return True`). Active-code AST gate (substrate.factories.parse_introduced): a docstring/comment
