...
owner confirmation once this map is green, per DESIGN DECISION 25) is gated on every one of
...
legitimate and must be told apart, one per Assay test FILE (`assay/tests/*.py`):
  ...
  * EXCLUDED  -- ruled out of the merge's scope by name, citing the real decision/entry that
                 ruled it out (DESIGN DECISION 26, or SPEC-5's own original cut-list for the
...
This is the map DESIGN DECISION 26 requires: `content_breach`/`stale_provenance`/`drift` (and
...
_FD26 = "DECISION 26"   # verbatim substring of the plan doc's decision heading
...
PARITY: dict = {
    ...
    "test_ledger.py": {
        ...
        "note": ("Concurrent-append lock + verify_chain over Assay's own hash-chained JSONL "
                 "store -- the same substrate gap DESIGN DECISION 26 ruled on for Task 6; "
