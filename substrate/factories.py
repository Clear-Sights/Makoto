...
def scan_target_content(tool_input: dict) -> str:
    ...
    (an AI could insert `.startswith(` into a verifier via Edit and slip past 1.1) closed
...
def is_false_const(node) -> bool:
    ...
    Shared by the ``verify=False`` / ``check_hostname=False`` keyword detectors (1.26 TLS, 1.28 JWT)."""
    return isinstance(node, ast.Constant) and node.value is False


def is_cert_none(node) -> bool:
    """True iff `node` is ``ssl.CERT_NONE`` (Attribute) or a bare ``CERT_NONE`` Name. Shared by the
    cert-disable detectors: 1.29 (``verify_mode = CERT_NONE`` assign) and 1.33 (``cert_reqs=CERT_NONE`` kwarg)."""
...
def callee_chain(call: ast.Call) -> str:
    ...
    Shared by the library-callee-gated detectors (1.26 TLS, 1.28 JWT)."""
    parts: list = []
    f = call.func
    while True:
