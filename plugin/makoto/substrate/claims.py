...
def whole_suite_pass_claim(text: str):
    ...
    (pass / green), and not negated, forward-framed, or quoted from code/log output. The match
    POSITION additionally feeds stale_pass's teeth-framing window.

    The subset firewall is the head-vs-modifier split: the word immediately before the subject
    head must be a universal modifier (`the`/`all`/`every`/…) or absent. 'parser tests pass',
    'these tests pass' fail open — they assert a SLICE, not the whole suite. The walk-back is
    CLAUSE-anchored (see _scoped_head), so a bare head after any prose still fires, and a
    partitive reached through function words ('some OF the tests pass') stays a subset. The
    predicate carries a right boundary (_PRED_TRAIL_RX), so an attributive or verb-complement use
    ('the build passes arguments to pytest') is not a green claim.

    named_test's named-subject machinery deliberately does NOT converge here: its negation/forward
    lexicons differ by measured design — merging would change verdicts."""
    if not text:
        return None
    ...
    for m in _GREEN_CLAIM_RX.finditer(text):
        ...
        # Scans the whole preceding sentence, not just the last comma-clause: a fronted
        # conditional ('Once you rebase, all tests pass.') is punctuated at exactly that comma.
...
def _scoped_head(text: str, a: int, subj: str) -> bool:
    ...
    Walks the head's contiguous pre-modifier run (clause/line-bounded, so a previous sentence
    never leaks in). A DIGIT ('244 tests passing') is an ENUMERATED count, not a universal claim.
    A non-universal WORD touching the head ('parser tests') scopes a SUBSET. A PARTITIVE reached
    through universal tokens ('some/most/half/3 OF the tests') scopes a subset too — 'of' hands
    scope to its left neighbour, which only 'all'/'every' keeps universal. Only a bare or
