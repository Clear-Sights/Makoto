...
- `gate.unverified_merge` — `gh pr merge` or a `git push` onto main/master with no clean verifier report settled before it (a run still in the background pays nothing); run the gate to a clean report, then merge.
- `content.loosened_after_red` — an Edit that loosens a test assertion (`==` to an inequality, or a bound moved the lenient way) while that test's recorded run is red; fix the code under test first.
- `event.owner_path` — `rm`/`mv`/`unlink`/`shred`/`truncate`/`git rm`, a Write, or a line-cutting Edit on a path `makoto.toml` declares in `owner_paths`; leave it for the owner.
- `event.repeated_append` — a Bash `>>` append rerun after the same command already succeeded with nothing touching the target since; overwrite with `>` or dedupe.
- `content.rule_without_runner` — a rule line (always / never / must / do not) added to CLAUDE.md or AGENTS.md that names no runner: add `runner: <path>` (or a path in the line) that exists.
- `content.check_without_pass_case` — a check row or predicate (`X_CHECK = Check(`, `def x_predicate(`, `def check_x(`) added to a checker file with no `pass: <test id or fixture path>` naming a file that exists.
- `content.overdetermined_case` — two or more verdict tests (named fires / blocks / denies / catch / flag / detect) added in one change without a distinct `discriminant:` each.
- `content.exemption_unnamed_region` — an exempt / exclude / skip / ignore / allow list added to a checker file with no `region:`, `unreached:` or NOT-COUNTABLE naming what it leaves unchecked.
- `content.fallthrough_match` — a Write/Edit to a `.py` file introducing a `match` whose last case is not a `case _:` that raises; end every dispatch in an error.
- `event.regime_unnamed` (opt-in: `makoto.toml` `require_regime = true`) — a `git commit`/`gh pr` message that removes code on a measured verdict with no `regime:` field; add `regime: <where it was measured>`.
...
- `gate.unworded_close` (end of turn, blocking; opt-in: `makoto.toml` `words_file = "VERIFY/WORDS.tsv"`) — a reply that says the work is closed/done citing no row of the words file (`WORDS.tsv:<line>` or a row id); cite the row you closed.
- `gate.unrun_count_claim` (end of turn, blocking) — the reply states a counted all-pass ("All 602 checks pass") and no verifier ran in the session; run it, then report its count.
