...
- The declared **Plan** (read by `gate.stale_establisher` and `gate.plan_item_drift`, both advisory) is declared by writing JSONL nodes (`{"what","passthrough","where"[,"id"]}` per line) to `.claude/makoto-plan.jsonl` — either staged on disk before the session starts, or via a live `Write`/`Edit`/`MultiEdit`/`NotebookEdit` to that same path mid-session (latest-wins: a later write replaces the whole Plan). A node advances to DONE automatically the moment a `Write`/`Edit`/`MultiEdit`/`NotebookEdit` targets that node's own `where`. No gate blocks on an open node: `gate.contract_order`, which did, was cut 2026-09-18 (no register entry named it).
