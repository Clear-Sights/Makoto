...
- Model-judged rows (plugin `hooks.json`, `type: "prompt"` PreToolUse hooks scoped by `if`): `makoto B7` on CLAUDE.md/AGENTS.md writes (a rule with no runner), `makoto B10` and `makoto B34` on `*check*.py` writes (a check that never abstains; a law exempting its own instrument), `makoto B21` on `test_*.py` writes (an overdetermined verdict). Each denies with the register's fix line as the reason.
