...
def _cmd_pattern_list() -> int:
    ...
    for p in patterns:
        ...
        print(f"{p.id:<6} {p.posture:<10} {_clip(kw, 30):<32} {_clip(p.description, 60)}")
    return 0
...
def _cmd_pattern_show(pid: str) -> int:
    ...
    if pid not in patterns:
        ...
        return 2
    p = patterns[pid]
    for label, value in (("id", p.id), ("posture", p.posture),
                         ("description", p.description), ("retry_hint", p.retry_hint),
                         ("predicate", p.predicate_module), ("keywords", p.keywords)):
