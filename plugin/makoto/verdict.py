...
# Only the two the Pre/Post renderers hardcode live here. The Stop-shaped renderer echoes back the
# firing hook name it is HANDED (``dispatch_posture``'s ``hook_name``), so it needs no constant of
# its own -- and the edge strings below already carry those two literals.
