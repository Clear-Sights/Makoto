...
Makoto must never normalize). The decided discharge: an OPERATOR-ATTRIBUTABLE `ack-block`
record, never prose, never atom-narrowing.
...
implements ONLY the host-transcript-attributed tier). The phrase a human types and the chain
`kind` both accept the NEW canonical form (`release.operator`) going forward AND the ORIGINAL
`ack-block` form FOREVER -- an id/phrase is never dead, only aliased, matching SPEC-C item 3's
own alias-table discipline applied here to a chain `kind` and a human-facing command phrase
rather than a check id.
...
A transcript entry is a valid ack iff it:
  ...
  5. contains the literal token `makoto ack-block <fingerprint-id>` (legacy, still honored
     forever) OR `makoto release.operator <fingerprint-id>` (current canonical form) plus a
     nonempty reason naming that exact id.
...
D8a's rename of the legacy kind="ack-block") for the receipt/audit trail, but the DISCHARGE
...
# D8a: (?:ack-block|release\.operator) accepts EITHER phrase permanently -- the legacy phrase
# an existing operator already knows must never silently stop working."""
_SYNTHETIC_MARKERS = (
...
def find_ack_block(fingerprint_id: str, *, transcript_path: Optional[str],
    ...
    """Scan the host-written transcript at `transcript_path` for a qualifying ack-block turn for
...
# D8a: the SET of chain kinds that mean "an operator-attributed release" -- both the legacy
# ("ack-block") and current canonical ("release.operator") forms, so a chain written before this
# rename and a chain written after it dedupe against EACH OTHER, never double-recording the same
# discharge just because the kind string changed underneath it.
_RELEASE_OPERATOR_KINDS = frozenset({"ack-block", "release.operator"})


def record_ack_block_if_new(ack: dict, *, session_id: Optional[str] = None,
                            root: Optional[Path] = None) -> bool:
    """Chain-append a `release.operator` row for `ack` (kind="release.operator", D8a's rename of
    the legacy kind="ack-block") UNLESS this exact (fingerprint_id, session_id) pair is already
    recorded under EITHER kind -- avoids flooding the chain with a duplicate row on every
