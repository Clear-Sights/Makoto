...
- **A tool-call** — the call the assistant is about to emit (`tool_input`, read at PreToolUse).
  ...
  own Write/Edit/MultiEdit (`scan_target_content`, `substrate/factories.py`).
...
a user's action is never misread as the AI's promise (`session/commitments.py:62-67`).
...
| green-claim gate (`stopchecks/stopcheck_green_claim.py`; reads the test-run ledger, `record/ledger.py`) | "the tests pass" | MISREPORTED (claim vs a recorded test *failure*) | **COVERED** — claim-vs-ledger: blocks "suite green" when the ledger recorded a failing run. |
| advance gate (`stopchecks/stopcheck_advance.py`) | "all done" | SELF-CONTRADICTING (over an open promise) | **COVERED** (flipped live 2026-06-01; 0-FP across all 1335 corpus sessions after the proposal-menu / code-fence sourcing guards, `session/commitments.py`; unified under `_gates_enabled()`, `_dispatch.py`) — every residual FP traced to a never-built PROPOSAL, not a genuine commitment (§7.6, closed). |
...
| `reconcile` + `subject_binds` (`verdict/retraction.py`, `checks.py:42-47`) | a promise's *release* | (keptness firewall) | **COVERED in mechanism** — a commitment clears only by a reason-bound retraction whose result-key *equals* (normalized) the commitment location; a fakeexcuse file cannot stand in. The one genuinely keptness-deep core — and it feeds the advance gate, now live, so a reason-bound retraction is what lets honest re-prioritization clear a promise without a false block. |
...
1. **✓ CLOSED (2026-06-01) — discharge is now content-deep.** Was: `_discharged` returned true on a
   ...
   verified, not substance. Now: a Write records its stripped length in the ledger (`record/ledger.py`),
...
4. **Makoto does not guard its own comparators.** `pattern_1_1`'s target is
   ...
   loosened a comparator inside makoto's own `stopchecks/`/`verdict/retraction.py`/`checks.py`, 1.1 would not fire. The
   ...
   *Slated for a follow-up plan (against tag `gap-7.6-advance-live`): a **diff-aware** weakening detector —
   ...
   blanket target extension is rejected: the relocated clause-parsing in `stopchecks/`/`verdict/retraction.py` and `session/commitments.py` use
...
5. **✓ CLOSED (2026-06-01) — the anti-bypass word is now un-forgeable.** Was: `makoto_allowed`
   matched a bare `makoto-allow` substring anywhere (then `helpers.py`; the marker regex is now `core/lexicons.py`) — no reason, no word boundary —
...
6. **✓ CLOSED (2026-06-01) — SELF-CONTRADICTING is now enforced.** Was: the advance gate ran in
   ...
   **never-built PROPOSAL** the AI recommended ("## What's worth building / Add X", "Option A: …
   ...
   discharged when its file was touched). So the fix is pure sourcing-precision in `session/commitments.py`,
